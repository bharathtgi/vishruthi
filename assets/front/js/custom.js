    ...
    $(document).on('click', '.quick-view', function(){
        ...
        $(".quick-view-modal").load($(this).data('href'),function(response, status, xhr){
			if(status == "success"){
				...
				$this.find('.modal-header').show();
				$this.find('.modal-body').show();
				$('.owl-carousel-quick').owlCarousel({
					loop: false,
					dots: false,
					margin: 10,
					nav: true,
      ...
      $('.'+$selector).hide();
        $.get( $(this).data('href') , function( data ) {
            if(data == 0) {
				...
				$('.cart_page').html('<h3 style="text-align:center">Cart is Empty.</h3>');
...
// Add By ONE
      ...
      $(document).on("click", ".adding" , function(){
			...
			var tempitemid =  $(this).parent().parent().find('.tempitemid').val();
			...
			$.ajax({
				...
				success:function(data){ //alert(data);
					if(data == 0)  {
						...
						$("#prc"+tempitemid).html(data[2]);
						$("#prct"+itemid).html(data[2]);
						$("#cqt"+itemid).html(data[1]);
						$("#qty"+tempitemid).html(data[1]);
						$(".tax-amt").html(data[9]);
					}
				}
			});
...
// Reduce By ONE
      ...
      $(document).on("click", ".reducing" , function(){
        ...
        var size_price = $(this).parent().parent().find('.size_price').val();
        var stck = $("#stock"+itemid).val();
        var qty = $("#qty"+tempitemid).html();
        qty--;

        
        if(qty < 1)
         {
         $("#qty"+tempitemid).html("1");
         }
         else{
         $("#qty"+tempitemid).html(qty);
            $.ajax({
...
// Review Section
    ...
    $(document).on('click','.stars', function(){
      ...
      $('#rating').val($(this).data('val'));
		if ($(this).data('val') == 1){
			$("#starone").css("color","#d9145d");
			$("#startwo").css("color","gray");
			$("#starthree").css("color","gray");
			$("#starfour").css("color","gray");
			$("#starfive").css("color","gray");
		} else if ($(this).data('val') == 2){
			$("#starone").css("color","#d9145d");
			$("#startwo").css("color","#d9145d");
			$("#starthree").css("color","gray");
			$("#starfour").css("color","gray");
			$("#starfive").css("color","gray");
