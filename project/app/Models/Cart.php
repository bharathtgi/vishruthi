...
// **************** ADD TO CART MULTIPLE *******************
    ...
    public function addnum($item, $id, $qty, $size, $color, $size_qty, $size_price, $size_key, $keys, $values, $chaintype, $banglesize) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values,'item_price' => $item->price, 'chaintype' => $item->chaintype, 'banglesize' => $item->banglesize];
        if($item->type == 'Physical')
        {
            if ($this->items) {
                if (array_key_exists($id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values), $this->items)) {
                    $storedItem = $this->items[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)];
                }
            }            
        }
        else {
        ...
        if(!empty($chaintype)){
			$storedItem['chaintype'] = $chaintype;    
        }
  
        if(!empty($banglesize)){
        $storedItem['banglesize'] = $banglesize;    
        }		
        if(!empty($keys)){
        $storedItem['keys'] = $keys;    
        }
        ...
        if(!empty($item->whole_sell_qty))
        {
            ...
            //     if($storedItem['qty'] == $whole_sell_qty)
            //     {   
            //         $whole_discount[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)] = $whole_sell_discount;
            //         Session::put('current_discount',$whole_discount);
            //         break;
            //     }                  
            // }
            ...
            foreach($item->whole_sell_qty as $key => $data){
                if(($key + 1) != count($item->whole_sell_qty)){
                    if(($storedItem['qty'] >= $item->whole_sell_qty[$key]) && ($storedItem['qty'] < $item->whole_sell_qty[$key+1])){
                        $whole_discount[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)] = $item->whole_sell_discount[$key];
                        Session::put('current_discount',$whole_discount);
                        $ck = 'first';
                        break;
                    }
            ...
            if(Session::has('current_discount')) {
                    $data = Session::get('current_discount');
                if (array_key_exists($id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values), $data)) {

                    $discount = $item->price * ($data[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)] / 100);
                    $item->price = $item->price - $discount;
                }
            }
        }
        ...
        $storedItem['price'] = $item->price * $storedItem['qty'];
        $this->items[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)] = $storedItem;
...
public function adding($item, $id, $size_qty) {
    $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => '', 'values' => '','item_price' => $item->price, 'chaintype' => $item->chaintype, 'banglesize' => $item->banglesize];
    if ($this->items) {
