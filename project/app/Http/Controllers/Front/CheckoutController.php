...
// If a user is Authenticated then there is no problm user can go for checkout
        ...
        if(Auth::guard('web')->check())
        {
				...
				// Vishruthi Shipping and Tax
				$tax_amount = round($tax,2);
				$tax_amount = number_format((float)($tax_amount), 2, '.', '');
				$total = number_format((float)($total), 2, '.', '');
				// Vishruthi Shipping and Tax
        return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 
										'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 
										'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 
										'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id,
...
// If guest checkout is Deactivated then display pop up form with proper error message
				...
				// Vishruthi Shipping and Tax				
				return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 
										'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'checked' => $ck, 'digital' => $dp, 
										'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 
										'vendor_packing_id' => $vendor_packing_id,'tax_amount' => $tax_amount]);                 
                    }
        }
...
// If a user is Authenticated then there is no problm user can go for checkout
        ...
        return view('front.checkout_s', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);             
        }
           	...
           	if($gs->guest_checkout == 1)
              {
                ...
                foreach ($products as $prod) {
                    if($prod['item']['type'] != 'Physical')
                    {
                        ...
                        {
                $ck = 1;
        return view('front.checkout_s', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'checked' => $ck, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);  
                        }
                    }
                }
        return view('front.checkout_s', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);                 
...
$validator = Validator::make($input, $rules, $messages);
	...
	public function  orderemail(){
		$data = [
			...
			$order = Order::findOrFail('81');
			$cart = json_decode($order->cart,true);
			//echo $order->currency_sign;
			//$currency = Currency::where('sign',$order->currency_sign)->first();
			//print_r($currency);  exit();			
			$mailer = new GeniusMailer();
			$mailer->sendAutoOrderMail($data,$order,$cart);
	}
}
