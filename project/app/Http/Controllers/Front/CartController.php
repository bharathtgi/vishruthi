...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        return view('front.cart', compact('products','totalPrice','mainTotal','tx','tax_amount')); 
    }
    public function cart_s()
    {
        $this->code_image();
        if (!Session::has('cart')) {
            return view('front.cart');
        ...
        return view('front.cart_s', compact('products','totalPrice','mainTotal','tx')); 
    }	
   ...
   public function addcart($id)
    ...
    public function addnumcart()
    {
        ...
        $values = $_GET['values'];
        $prices = $_GET['prices'];
		$chaintype = $_GET['chaintype'];
		$banglesize = $_GET['banglesize'];
        ...
        $prod = Product::where('id','=',$id)->first(['id','user_id','slug','name','photo','size','size_qty','size_price','color','price','stock','type','file','link','license','license_qty','measure','whole_sell_qty','whole_sell_discount','attributes','chaintype','banglesize']);
        ...
        $oldCart = Session::has('cart') ? Session::get('cart') : null;
        $cart = new Cart($oldCart);
        $cart->addnum($prod, $prod->id, $qty, $size,$color,$size_qty,$size_price,$size_key,$keys,$values,$chaintype,$banglesize);
        if($cart->items[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)]['dp'] == 1)
        {
            return 'digital';
        }
        if($cart->items[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)]['stock'] < 0)
        {
            return 0;
        }
        if($cart->items[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)]['size_qty'])
        {
            if($cart->items[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)]['qty'] > $cart->items[$id.$size.$color.$chaintype.$banglesize.str_replace(str_split(' ,'),'',$values)]['size_qty'])
            {
                return 0;
            }           
        }
    ...
    public function removecart($id)
    {
        ...
        if (count($cart->items) > 0) {
			...
			//Here we can Reduce the amount of coupon
			if($gs->currency_format == 0){
