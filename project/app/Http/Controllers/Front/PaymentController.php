    ...
    // Redirect to paypal IPN
     ...
     public function payreturn(){
        ...
        if(Session::has('temporder_id')){
			...
			if ( ($order->user_id != '') && ($order->user_id != NULL) ){
				$productid_arr = array();
				foreach($tempcart['items'] as $product){
					$productid_arr[] = $product['item']['id'];
				}
				$wishlist_chk = Wishlist::where('user_id','=',$order->user_id)->whereIn('product_id', $productid_arr)->count('id');
				if ($wishlist_chk > 0){
					Wishlist::where('user_id','=',$order->user_id)->whereIn('product_id', $productid_arr)->delete();
				}
			}
		...
		if($gs->is_smtp == 1)          {
	      $data = [
			...
			$order_dt = Order::findOrFail($order->id);
			$cart_dt = json_decode($order_dt->cart,true);
			$mailer = new GeniusMailer();
			$mailer->sendAutoOrderMail($data,$order_dt,$cart_dt);      
/* 			 $data = [
				'to' => $order->customer_email,
				'subject' => "Your Order Placed!!",
				'body' => "Hello ". $order->customer_name ."!<br>You have placed a new order.\nYour order number is ". $order->order_number .".Please wait for your delivery. <br>Thank you."
			];
	...
	public function payreturnnew(){		
		return view('front.successnew');
	}
