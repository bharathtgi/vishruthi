...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
      ...
      $maxprice = round(($maxprice / $curr->value),2);
		$filter_subcat = $request->filter_subcat;
		$is_antic = $request->is_antic;
		$filter_chaintype = $request->filter_chaintype;
		$filter_banglesize = $request->filter_banglesize;
      if (!empty($slug)) {
      ...
      $prods = Product::when($cat, function ($query, $cat) {
                                  ...
                                  $prods = $prods->where(function ($query) use ($cat, $subcat, $childcat, $request, $filter_subcat, $filter_chaintype, $filter_banglesize) {
                                              $flag = 0;
                                              ...
                                              if (!empty($childcat)) {
												...
												if ($filter_subcat != ''){
													$filter_subcat_arr = explode(",",$filter_subcat);
													$query->whereIn('subcategory_id', $filter_subcat_arr);
												}
												if ($filter_chaintype != ''){
 													$filter_chaintype_arr = explode(",",$filter_chaintype);
													$query->where (function ($subquery) use ($filter_chaintype_arr) {
														for($n=0;$n<count($filter_chaintype_arr);$n++){
															if ($n == 0){
																$subquery->where('chaintype', 'like', '%'. $filter_chaintype_arr[$n] .'%');
															} else {
																$subquery->orWhere('chaintype', 'like', '%'. $filter_chaintype_arr[$n] .'%');
															}
														}
													}); 
													//$query->where('chaintype', 'like', '%'. $filter_chaintype .'%');
												}
												if ($filter_banglesize != ''){
													$filter_banglesize_arr = explode(",",$filter_banglesize);
													$query->where (function ($subquery) use ($filter_banglesize_arr) {
														for($n=0;$n<count($filter_banglesize_arr);$n++){
															if ($n == 0){
																$subquery->where('banglesize', 'like', '%'. $filter_banglesize_arr[$n] .'%');
															} else {
																$subquery->orWhere('banglesize', 'like', '%'. $filter_banglesize_arr[$n] .'%');
															}
														}
													});
												}												
										...
										if (!empty($is_antic)) {
											$prods = $prods->where(function ($query) use ($is_antic) {
												$query->where('is_antic', '=', 1);
											});
										}
    ...
    public function category_s(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
      if (Session::has('currency')) 
      {
        $curr = Currency::find(Session::get('currency'));
      ...
      $data['ajax_check'] = 1;
        ...
        return view('includes.product.filtered-products_s', $data);
      }
      return view('front.category_s', $data);
    }
		...
		// Vishruthi Update
 		if (Session::has('cart')) {
			...
			foreach($products as $prod){ 
				if ($prod['item']['id'] == $productt->id){
					$productt->stock = $productt->stock - $prod['qty'];
				}
			}
		} 
		// Vishruthi Update		
    ...
    public function product_s($slug)
    {
        $this->code_image();
        $productt = Product::where('slug','=',$slug)->firstOrFail();
        if($productt->status == 0){
		...
		// Vishruthi Update		
        return view('front.product_s',compact('productt','curr','vendors'));
...
//        $product->update();
    ...
    // ------------------ Rating SECTION --------------------
        ...
        public function reviews_s($id){
            $productt = Product::find($id);
            return view('load.reviews_s',compact('productt','id'));
        }		
