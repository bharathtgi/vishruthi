      ...
      <div class="col-lg-8">
              ...
              @include('includes.form-success')
                <thead>
                  ...
                  <tbody>
                    ...
                    @endphp
                    <tr class="cremove{{ $product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values']) }}">
                          ...
                          <p class="name"><a href="{{ route('front.product', $product['item']['slug']) }}">{{mb_strlen($product['item']['name'],'utf-8') > 35 ? mb_substr($product['item']['name'],0,35,'utf-8').'...' : $product['item']['name']}}</a></p>
                                            ...
                                            <td>
												...
												@if(!empty($product['chaintype']))
													<b>Chain Type</b> : {{$product['chaintype']}}
												@endif
												@if(!empty($product['banglesize']))
													<b>Bangle Size</b> : {{$product['banglesize']}}
												@endif
          ...
          @if($product['item']['type'] == 'Physical')
                          ...
                          <div class="qty">
                              <ul>
                                ...
                                <li><span class="qttotal1" id="qty{{$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])}}">{{ $product['qty'] }}</span></li>
        ...
        @endif
                            ...
                            <td class="total-price">
                              <p id="prc{{$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])}}">
                                {{ App\Models\Product::convertPrice($product['price']) }}                 
                              </p>
      
                              <small id="prev{{$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])}}" class="{{Session::has('current_discount'.$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values']))  && isset(Session::get('current_discount'.$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values']))[$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])]) ? '' : 'd-none'}}">(Discount <small id="discount{{$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])}}">{{Session::has('current_discount'.$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values']))  && isset(Session::get('current_discount'.$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values']))[$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])])   ? Session::get('current_discount'.$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values']))[$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])]   : '' }}</small>%)</small>
                            </td>
                      <td>
                        <span class="removecart cart-remove" data-class="cremove{{ $product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values']) }}" data-href="{{ route('product.cart.remove',$product['item']['id'].$product['size'].$product['color'].$product['chaintype'].$product['banglesize'].str_replace(str_split(' ,'),'',$product['values'])) }}"><i class="icofont-ui-delete"></i> </span>
                      </td>
                    </tr>
                    @endforeach
                    @endif
